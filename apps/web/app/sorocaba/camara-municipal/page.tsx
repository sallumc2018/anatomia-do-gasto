...
function latestCamaraFile(prefix: string): string {
  const dir = path.join(DATA_ROOT, "sorocaba", "camara", "saida")
  const files = fs.existsSync(dir)
    ? fs.readdirSync(dir).filter(f => f.startsWith(prefix) && f.endsWith(".csv"))
    : []
  return files.sort().at(-1) ?? `${prefix}_2020_2026.csv`
}

function loadCamaraTce(): CamaraTceRow[] {
  const fp = path.join(DATA_ROOT, "sorocaba", "camara", "saida", latestCamaraFile("camara_despesas_tce"))
