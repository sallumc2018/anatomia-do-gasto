...
// Returns the latest multi-year aggregate file matching {prefix}_2020_20YY.csv,
// so the page doesn't break when a new year suffix is published.
function latestFile(prefix: string): string {
  const files = fs.existsSync(AUTARQUIAS)
    ? fs.readdirSync(AUTARQUIAS).filter(f => f.startsWith(prefix) && f.endsWith(".csv"))
    : []
  return files.sort().at(-1) ?? `${prefix}_2020_2026.csv`
}
...
export default function AutarquiasPage() {
  const saaeDesp     = aggregateTransacoes(latestFile("saae_despesas_tce"))
  const saaeRec      = aggregateTransacoes(latestFile("saae_receitas_tce"))
  const funservDesp  = aggregateTransacoes(latestFile("funserv_saude_tce"))
  const empresasDesp = aggregateTransacoes(latestFile("empresas_municipais_tce"))
  const rpps         = loadRpps()
