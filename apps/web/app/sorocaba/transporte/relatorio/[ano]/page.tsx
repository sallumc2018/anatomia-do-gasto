...
export async function generateMetadata({ params }: { params: Promise<{ ano: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Despesas em transporte — ${ano} | Sorocaba`,
    description: `Execução orçamentária em transporte de Sorocaba em ${ano}: dotação atualizada, empenhada, liquidada e paga. Fonte: DCA e relatórios publicados pela Prefeitura de Sorocaba/SP.`,
    alternates: { canonical: `https://www.anatomiadogasto.ong.br/sorocaba/transporte/relatorio/${ano}` },
  }
}
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        description: `Execução orçamentária em transporte de Sorocaba em ${year}: dotação atualizada, empenhada, liquidada e paga. Fonte: DCA e relatórios publicados pela Prefeitura de Sorocaba/SP.`,
        url: `https://www.anatomiadogasto.ong.br/sorocaba/transporte/relatorio/${year}`,
      ...
      <main id="conteudo" className="flex-1">
        ...
        <div style={{ backgroundColor: "var(--bg-elevated)", borderBottom: "1px solid var(--border-01)" }}>
          <div className="mx-auto px-6 py-3" style={S.container}>
            <nav style={S.caption}>
              <Link href="/sorocaba/transporte" style={{ color: "var(--blue-60)" }}>Transporte</Link>
              {" › "}
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)" }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
            ...
            <div className="flex items-center gap-3 mt-5">
              ...
              <a
                href={`/api/dados/sorocaba/transporte/saida/rreo_transporte_sorocaba_${year}.csv`}
                download
              ...
              <a
                href={`/api/dados/sorocaba/transporte/saida/dca_transporte_sorocaba_${year}.csv`}
                download
                style={{ fontSize: "12px", fontFamily: "var(--font-ibm-plex-mono)", color: "var(--blue-40)", textDecoration: "none", border: "1px solid var(--border-01)", padding: "4px 10px", whiteSpace: "nowrap" }}
              >
                ↓ DCA
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)", borderTop: "1px solid var(--border-01)" }}>
          <div className="mx-auto px-6 py-6 flex justify-between items-center" style={S.container}>
            {hasPrev ? (
              <Link href={`/sorocaba/transporte/relatorio/${prevYear}`}
                    style={{ color: "var(--blue-60)", fontSize: "14px" }}>
                ← {prevYear}
              </Link>
            ) : <span />}
            <Link href="/sorocaba/transporte/comparativo"
                  style={{ color: "var(--text-03)", fontSize: "13px" }}>
              Ver comparativo histórico
            </Link>
            {hasNext ? (
              <Link href={`/sorocaba/transporte/relatorio/${nextYear}`}
                    style={{ color: "var(--blue-60)", fontSize: "14px" }}>
                {nextYear} →
              </Link>
            ) : <span />}
