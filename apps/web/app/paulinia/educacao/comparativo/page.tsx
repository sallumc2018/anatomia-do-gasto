...
export const metadata: Metadata = {
  title: "Educação em Paulínia — série histórica do MDE",
  description:
    "Série histórica do Mínimo de Desenvolvimento do Ensino (MDE) em Paulínia: percentual aplicado, limite constitucional de 25% e situação de cumprimento por ano. Fonte: SICONFI/Tesouro Nacional — RREO Anexo 14.",
  alternates: { canonical: `${SITE_URL}/paulinia/educacao/comparativo` },
}
...
function loadSiopeAno(ano: number): SiopeAno | null {
  const filePath = path.join(DATA_ROOT, "paulinia", "educacao", "saida", `siope_paulinia_${ano}.csv`)
...
const jsonLd = [
  datasetSchema({
    name: "Mínimo de Desenvolvimento do Ensino (MDE) — Paulínia",
    description:
      "Série histórica do indicador constitucional de MDE do Município de Paulínia: percentual aplicado, limite constitucional (25%) e situação de cumprimento, por ano. Fonte: RREO Anexo 14 / SICONFI. IBGE 3536505.",
    url: `${SITE_URL}/paulinia/educacao/comparativo`,
    temporalCoverage: `${ANOS[0]}/${ANOS[ANOS.length - 1]}`,
    spatialCoverage: "Paulínia, SP, Brasil (IBGE 3536505)",
    keywords: ["educação", "Paulínia", "SICONFI", "MDE", "RREO", "Anexo 14"],
    dateModified: "2026-07-09",
    downloadUrls: ANOS.map(
      (ano) => `${SITE_URL}/api/dados/paulinia/educacao/saida/siope_paulinia_${ano}.csv`,
    ),
  }),
  breadcrumbSchema([
    { name: "Início", url: SITE_URL },
    { name: "Paulínia", url: `${SITE_URL}/paulinia` },
    { name: "Educação", url: `${SITE_URL}/paulinia/educacao` },
    { name: "Série histórica" },
  ]),
]

export default function ComparativoEducacaoPauliniaPage() {
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)", ...S.borderBottom }}>
          <div className="mx-auto px-6 py-14 md:py-20" style={S.container}>
            <div className="flex items-center gap-3 mb-4">
              <Link href="/paulinia/educacao" style={{ ...S.small, color: "var(--text-03)", textDecoration: "none" }}>Educação</Link>
            ...
            <div style={{ borderLeft: "4px solid var(--green-60)", paddingLeft: "24px" }}>
              <p className="uppercase font-semibold mb-3" style={S.label}>
                Educação · Paulínia/SP · {ANOS[0]}–{ANOS[ANOS.length - 1]}
              </p>
              <h1 className="font-light mb-5" style={{ fontSize: "clamp(26px, 3.5vw, 42px)", lineHeight: "1.2", color: "var(--text-01)", maxWidth: "720px" }}>
                O cumprimento do mínimo constitucional de educação em Paulínia, ano a ano
              </h1>
              <p style={{ ...S.body, fontSize: "15px", lineHeight: "24px", maxWidth: "640px" }}>
                ...
                Ensino (MDE) de Paulínia: o percentual de receitas de impostos aplicado em educação
          ...
          <div className="mx-auto px-6 py-14" style={S.container}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="p-6" style={{ backgroundColor: "var(--bg-elevated)", border: "1px solid var(--border-01)" }}>
                ...
                <p style={S.body}>
                  Nem todo ano tem indicador disponível na fonte oficial (SICONFI). Quando a situação
                  aparece como <strong style={{ color: "var(--text-01)" }}>&ldquo;não coletado&rdquo;</strong>,
                  significa que a informação não está publicada pelo ente — não que o percentual
                  aplicado foi zero. O Anatomia do Gasto nunca preenche uma lacuna com zero.
                </p>
              </div>
            </div>
          </div>
                      ...
                      <tr key={row.ano} style={{ borderBottom: "1px solid var(--border-01)" }}>
                        ...
                        <td style={{ padding: "14px 0 14px 16px", textAlign: "right", whiteSpace: "nowrap" }}>
                          ...
                          {!semDado && (
                            <>
                              {" "}
                              <Link href={`/paulinia/educacao/relatorio/${row.ano}`} style={{ fontSize: "12px", color: "var(--blue-50)", textDecoration: "none" }}>
                                Ver {row.ano} →
                              </Link>
                            </>
                          )}
            ...
            {anosSemDado.length > 0 && (
              <div className="mt-6 p-4" style={{ backgroundColor: "var(--bg-base)", border: "1px solid var(--border-01)" }}>
                <p style={{ ...S.small, color: "var(--text-03)" }}>
                  ...
                  no SICONFI, não uma falha de coleta do Anatomia do Gasto.
                </p>
              </div>
            )}
          </div>
        ...
        <section style={{ backgroundColor: "var(--bg-base)", ...S.borderBottom }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
            ...
            <p style={{ ...S.body, maxWidth: "640px" }}>
              ...
              (PDDE, PNAE, PNATE, FUNDEB) para Paulínia: não existe hoje uma fonte pública
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)", ...S.borderBottom }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div>
                ...
                <div className="mt-2 flex flex-col gap-2">
                  ...
                  <Link href="/paulinia/educacao" style={{ fontSize: "13px", color: "var(--blue-50)", textDecoration: "none" }}>
            ...
            <p className="mt-6" style={S.caption}>
              Município de Paulínia — IBGE 3536505 · população 106.773. Valores nominais em BRL.
            </p>
          </div>
        </section>
            ...
            <div className="mx-auto px-6 py-12" style={S.container}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                {[...anosComDado].reverse().map((row) => (
                  <Link key={row.ano} href={`/paulinia/educacao/relatorio/${row.ano}`}
                    className="tile-link"
