...
const SUBFUNCAO_TIPS: Record<string, string> = {
  ...
  "06.181 - Policiamento":
    "Guarda Civil Municipal: patrulhamento, atendimento de ocorrências e vigilância de espaços públicos.",
  "06.182 - Defesa Civil":
  ...
  "06.999 - Demais Subfunções Segurança Pública":
    "Subfunção residual declarada pelo DCA, sem detalhamento adicional na fonte.",
}
...
export async function generateMetadata({ params }: { params: Promise<{ ano: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Despesas em segurança pública — ${ano} | São Bernardo do Campo`,
    description: `Execução orçamentária em segurança pública de São Bernardo do Campo em ${ano}: dotação atualizada, empenhada, liquidada e paga. Fonte: DCA Anexo I-E e RREO Anexo 02 — SICONFI.`,
    alternates: { canonical: `https://www.anatomiadogasto.ong.br/sao-bernardo/seguranca/relatorio/${ano}` },
  }
}
...
export default async function RelatorioSegurancaPage({ params }: PageProps) {
  ...
  const availableYears = getAvailableYearsSeguranca(MUNICIPIO)
  if (!availableYears.includes(year)) notFound()

  const rows  = loadSegurancaData(year, MUNICIPIO)
  const total = rows.find((r) => r.subfuncao === SEGURANCA_TOTAL)
  const detail = rows.filter((r) => r.subfuncao !== SEGURANCA_TOTAL)
  const declaredSum = detail.reduce((acc, r) => acc + r.liquidada, 0)
  const residual = (total?.liquidada ?? 0) - declaredSum
  const hasResidual = residual > 1

  const prevYear = availableYears.find((y) => y < year) ?? null
  const prevRows = prevYear ? loadSegurancaData(prevYear, MUNICIPIO) : []
  const prevBySubfuncao: Record<string, number> = Object.fromEntries(
    prevRows.map((r) => [r.subfuncao, r.liquidada])
  )
  const prevDeclaredSum = prevRows.filter((r) => r.subfuncao !== SEGURANCA_TOTAL).reduce((acc, r) => acc + r.liquidada, 0)
  const prevTotal = prevBySubfuncao[SEGURANCA_TOTAL]
  const prevResidual = prevTotal !== undefined ? prevTotal - prevDeclaredSum : undefined
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <JsonLd data={{
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Segurança Pública", item: "https://www.anatomiadogasto.ong.br/sao-bernardo/seguranca" },
          { "@type": "ListItem", position: 3, name: "Série histórica", item: "https://www.anatomiadogasto.ong.br/sao-bernardo/seguranca/comparativo" },
      ...
      <JsonLd data={{
        "@context": "https://schema.org",
        "@type": "Dataset",
        name: `Execução orçamentária em segurança pública — São Bernardo do Campo ${year}`,
        description: `Execução orçamentária em segurança pública de São Bernardo do Campo em ${year}: dotação atualizada, empenhada, liquidada e paga por subfunção. Fonte: DCA Anexo I-E e RREO Anexo 02 — SICONFI.`,
        url: `https://www.anatomiadogasto.ong.br/sao-bernardo/seguranca/relatorio/${year}`,
      ...
      <main id="conteudo" className="flex-1">
        ...
        <div style={{ backgroundColor: "var(--bg-base)", ...S.borderBottom }}>
          <div className="mx-auto px-6 py-3 flex items-center gap-2" style={{ ...S.container, flexWrap: "wrap" }}>
            ...
            <Link href="/sao-bernardo/seguranca" className="nav-link" style={{ fontSize: "12px" }}>Segurança</Link>
            <span style={{ fontSize: "12px", color: "var(--text-04)" }}>/</span>
            <Link href="/sao-bernardo/seguranca/comparativo" className="nav-link" style={{ fontSize: "12px" }}>Série histórica</Link>
            <span style={{ fontSize: "12px", color: "var(--text-04)" }}>/</span>
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                <p className="uppercase font-semibold mb-3" style={S.label}>
                  São Bernardo do Campo / SP · Segurança Pública · Exercício completo
                </p>
              ...
              <div className="mobile-scroll flex items-center gap-3">
                {availableYears.map((y) => (
                  <Link key={y} href={`/sao-bernardo/seguranca/relatorio/${y}`} style={{
                    fontSize: "13px",
                ...
                <a
                  href={`/api/dados/sao_bernardo/seguranca/saida/despesas_seguranca_sao_bernardo_${year}.csv`}
                  download
          ...
          <section style={{ backgroundColor: "var(--bg-elevated)", ...S.borderBottom }}>
            <div className="mx-auto px-6 py-8" style={S.container}>
              <p className="uppercase font-semibold mb-6" style={S.label}>
                DCA Anexo I-E · Exercício {year} · São Bernardo do Campo/SP
              </p>
                ...
                <div className="mt-6" style={{ border: "1px solid var(--border-01)" }}>
                  ...
                  <div className="px-4 py-3" style={{ borderTop: "1px solid var(--border-01)", backgroundColor: "var(--bg-base)" }}>
                    <p style={{ ...S.caption, lineHeight: "18px" }}>
                      ...
                      Em São Bernardo do Campo, o Empenhado e o Liquidado do DCA coincidem exatamente com o
                      Empenhado e o Liquidado EXCETO INTRA do RREO em todos os exercícios publicados — não há
                      divergência de consolidação neste ano.
        ...
        {/* ── Nota: parcela não detalhada por subfunção ─────────────────────── */}
        {hasResidual && (
            ...
            <div className="mx-auto px-6 py-6" style={S.container}>
              <div className="p-5" style={{ backgroundColor: "var(--bg-elevated)", border: "1px solid var(--border-01)", borderLeft: "4px solid #f1c21b" }}>
                <p className="font-semibold mb-2" style={{ color: "var(--text-01)", fontSize: "13px" }}>
                  Parte do gasto de {year} não é detalhada por subfunção
                </p>
                <p style={{ ...S.small, color: "var(--text-03)" }}>
                  A soma das subfunções declaradas no DCA de {year} ({formatMillions(declaredSum)}) é menor que o
                  total liquidado da função 06 ({formatMillions(total?.liquidada ?? 0)}). A diferença de{" "}
                  {formatMillions(residual)} ({pct(residual, total?.liquidada ?? 0)} do total) está incluída na
                  linha &ldquo;Não detalhado por subfunção&rdquo; da tabela abaixo. Não é uma subfunção real —
                  é a parcela do gasto que o DCA não atribui a nenhuma linha específica neste exercício.
                </p>
              </div>
            </div>
          </section>
        ...
        <section style={{ backgroundColor: "var(--bg-base)" }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
              ...
              <div className="table-scroll">
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    ...
                    {hasResidual && (
                      <tr key={RESIDUAL_KEY} style={{ ...S.borderBottom, fontStyle: "italic" }}>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", color: "var(--text-03)" }}>
                          <span title="Diferença entre o total declarado da função 06 e a soma das subfunções detalhadas no DCA deste exercício.">
                            Não detalhado por subfunção
                          </span>
                        </td>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", textAlign: "right", color: "var(--text-04)" }}>—</td>
                        <NumCell value={residual} />
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", textAlign: "right", color: "var(--text-04)" }}>—</td>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", textAlign: "right", color: "var(--text-03)", fontFamily: "var(--font-ibm-plex-mono)" }}>
                          {total ? pct(residual, total.liquidada) : "—"}
                        </td>
                        {prevYear && (() => {
                          const delta = prevResidual !== undefined ? fmtDelta(residual, prevResidual) : { text: "—", up: null }
                          return (
                            <td style={{
                              padding: "14px 0 14px 0",
                              fontSize: "12px",
                  ...
                  {total && (
                    <tfoot>
                      <tr style={{ borderTop: "2px solid var(--border-02)" }}>
                        ...
                        {prevYear && (() => {
                          const prevTotalVal = prevBySubfuncao[SEGURANCA_TOTAL]
                          const delta = fmtDelta(total.liquidada, prevTotalVal)
                          return (
                            <td style={{
                              padding: "14px 0 14px 0",
                              fontSize: "12px",
            ...
            <p className="mt-8" style={S.caption}>
              Fonte: DCA Anexo I-E — SICONFI / Tesouro Nacional. São Bernardo do Campo/SP · Exercício {year}.
              ...
              <div>
                ...
                <ul className="flex flex-col gap-3">
                  {[
                    ...
                    "A participação percentual de cada subfunção no total da função, incluindo a parcela não detalhada.",
