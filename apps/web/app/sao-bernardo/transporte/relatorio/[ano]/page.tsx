...
export async function generateMetadata({ params }: { params: Promise<{ ano: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Despesas em transporte — ${ano} | São Bernardo do Campo`,
    description: `Execução orçamentária em transporte de São Bernardo do Campo em ${ano}: dotação atualizada, empenhada, liquidada e paga. Fonte: SICONFI/Tesouro Nacional — RREO Anexo 02 e DCA Anexo I-E.`,
    alternates: { canonical: `https://www.anatomiadogasto.ong.br/sao-bernardo/transporte/relatorio/${ano}` },
  }
}
...
export default async function TransporteRelatorioPage({ params }: PageProps) {
  ...
  const inconsistente = dca && orcamento
    && Math.abs(dca.empenhado - orcamento.empenhado) > 1
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <JsonLd data={{
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Transporte", item: "https://www.anatomiadogasto.ong.br/sao-bernardo/transporte" },
      ...
      <JsonLd data={{
        "@context": "https://schema.org",
        "@type": "Dataset",
        name: `Execução orçamentária em transporte — São Bernardo do Campo ${year}`,
        description: `Execução orçamentária em transporte de São Bernardo do Campo em ${year}: dotação atualizada, empenhada, liquidada e paga. Fonte: SICONFI/Tesouro Nacional — RREO Anexo 02 e DCA Anexo I-E.`,
        url: `https://www.anatomiadogasto.ong.br/sao-bernardo/transporte/relatorio/${year}`,
      ...
      <main id="conteudo" className="flex-1">
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)" }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
            <p style={S.label} className="mb-3">Função 26 — Transporte · São Bernardo do Campo/SP</p>
            ...
            <p className="mt-4" style={{ fontSize: "13px", color: "var(--text-04)", maxWidth: "640px" }}>
              <strong style={{ color: "var(--text-03)" }}>Limitação desta fonte:</strong> São Bernardo do Campo
              declara toda a função 26 sem quebra por subfunção nestes dados. Os valores abaixo incluem
              transporte coletivo urbano, obras viárias e demais subfunções de mobilidade sem discriminação.
            </p>
            <div className="flex items-center gap-3 mt-5">
              <span style={{ fontSize: "11px", color: "var(--text-03)", fontFamily: "var(--font-ibm-plex-mono)" }}>Baixar dados brutos:</span>
              <a
                href={`/api/dados/sao_bernardo/transporte/saida/rreo_transporte_sao_bernardo_${year}.csv`}
                download
              ...
              <a
                href={`/api/dados/sao_bernardo/transporte/saida/dca_transporte_sao_bernardo_${year}.csv`}
                download
                style={{ fontSize: "12px", fontFamily: "var(--font-ibm-plex-mono)", color: "var(--blue-40)", textDecoration: "none", border: "1px solid var(--border-01)", padding: "4px 10px", whiteSpace: "nowrap" }}
              >
                ↓ DCA
          ...
          <div className="mx-auto px-6 py-12" style={S.container}>
            ...
            {dca ? (
              <>
                ...
                {inconsistente && orcamento && (
                  <p className="mb-6" style={{ fontSize: "13px", color: "var(--text-04)", maxWidth: "640px" }}>
                    <strong style={{ color: "var(--text-03)" }}>Divergência declarada em {year}:</strong>{" "}
                    o Empenhado do DCA ({formatPrecise(dca.empenhado)}) difere do Empenhado do RREO Anexo 02
                    do mesmo ano ({formatPrecise(orcamento.empenhado)}) em{" "}
                    {formatPrecise(Math.abs(dca.empenhado - orcamento.empenhado))}. A diferença é pequena
                    (bem abaixo de 0,01% do valor total) e reflete o dado declarado ao SICONFI em cada anexo,
                    sem correção local.
                  </p>
                )}
        ...
        <section style={{ backgroundColor: "var(--bg-base)" }}>
          <div className="mx-auto px-6 py-10" style={S.container}>
            ...
            <ul style={{ ...S.body, listStyle: "disc", paddingLeft: "24px", display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <li>Operação (frota, linhas, passageiros) — não há dados abertos publicados nesta fonte</li>
              <li>Contratos do serviço — requer curadoria adicional (PNCP, portal de transparência municipal)</li>
              <li>Serviços intermunicipais (EMTU/SP) — linhas metropolitanas do ABC Paulista operadas pelo Estado não estão incluídas no orçamento municipal</li>
            </ul>
          </div>
        </section>
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)", borderTop: "1px solid var(--border-01)" }}>
          <div className="mx-auto px-6 py-6 flex justify-between items-center" style={S.container}>
            {hasPrev ? (
              <Link href={`/sao-bernardo/transporte/relatorio/${prevYear}`}
                    style={{ color: "var(--blue-60)", fontSize: "14px" }}>
                ← {prevYear}
              </Link>
            ) : <span />}
            <Link href="/sao-bernardo/transporte/comparativo"
                  style={{ color: "var(--text-03)", fontSize: "13px" }}>
              Ver comparativo histórico
            </Link>
            {hasNext ? (
              <Link href={`/sao-bernardo/transporte/relatorio/${nextYear}`}
                    style={{ color: "var(--blue-60)", fontSize: "14px" }}>
                {nextYear} →
              </Link>
            ) : <span />}
