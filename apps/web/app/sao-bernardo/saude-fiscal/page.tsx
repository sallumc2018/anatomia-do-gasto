...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "https://www.anatomiadogasto.ong.br/sao-bernardo/saude-fiscal" },
}
...
export default function SaoBernardoSaudeFiscalPage() {
  ...
  const dividaSerie:  DividaRow[]  = anos.map((a) => loadDivida(a, MUNICIPIO)).filter((r): r is DividaRow => r !== null).sort((a, b) => a.ano - b.ano)
  ...
  const pessoalInsights: string[] = [
    ...(pessoalAtual ? [
      `Em ${anoAtual}, São Bernardo do Campo gastou ${pessoalAtual.dtp_pct_rcl.toFixed(2)}% da RCL ajustada com pessoal — o limite legal para o Executivo é 54% (LRF art. 20). A margem disponível é de ${(pessoalAtual.limite_maximo_pct - pessoalAtual.dtp_pct_rcl).toFixed(2)} pontos percentuais.`,
  ...
  const dividaInsights: string[] = [
    ...(dividaAtual ? [
      `A dívida consolidada bruta de SBC em ${anoAtual} era de ${fmt(dividaAtual.dc_bruta)} (${dividaAtual.dc_pct_rcl.toFixed(1)}% da RCL informada no RGF). O limite fixado pela Resolução do Senado Federal 40/2001 é de 120%.`,
      `A dívida consolidada líquida (DCL) foi de ${fmt(dividaAtual.dcl)} (${dividaAtual.dcl_pct_rcl.toFixed(2)}% da base).`,
    ] : []),
  ]
          ...
          <div className="mx-auto px-6 py-16 md:py-24" style={S.container}>
            <div className="mobile-hero-inset" style={{ borderLeft: "4px solid var(--blue-60)", paddingLeft: "24px" }}>
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <p className="uppercase font-semibold" style={S.label}>Saúde Fiscal · São Bernardo do Campo/SP</p>
              ...
              <p style={{ ...S.body, maxWidth: "640px", marginBottom: "12px" }}>
                ...
                e endividamento dos municípios. Em {anoAtual}, SBC gastou{" "}
          ...
          <div className="mx-auto px-6 py-12" style={S.container}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                { label: "Dívida consolidada bruta",    valor: dividaAtual  ? `${dividaAtual.dc_pct_rcl.toFixed(1)}%` : "—",  nota: "da RCL — limite: 120%" },
              ].map((item) => (
                <div key={item.label}>
                ...
                {pessoalAtual && (
                  <div className="mt-4 p-4" style={{ backgroundColor: "var(--bg-elevated)", border: "1px solid var(--border-01)" }}>
                    <p style={{ ...S.caption, lineHeight: "18px" }}>
                      ...
                      e {fmt(pessoalAtual.pessoal_inativo)} com inativos e pensionistas do RPPS.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="mx-auto px-6 py-12" style={S.container}>
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-12 items-start">
              <div>
                ...
                <p style={{ ...S.body, marginBottom: "16px" }}>
                  O limite máximo é 120% da RCL (Resolução do Senado 40/2001).
                  A DCL (líquida) desconta disponibilidades de caixa da dívida bruta.
                </p>
                <div style={S.borderTop}>
              ...
              <div>
                <p style={{ ...S.label, marginBottom: "12px" }}>DC bruta % da RCL — limite: 120%</p>
          ...
          <div className="mx-auto px-6 py-12" style={S.container}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                ...
                <p style={S.body}>Despesa com Pessoal — Relatório de Gestão Fiscal do Poder Executivo, periodicidade semestral.</p>
              </div>
              <div>
                <p style={{ fontSize: "14px", fontWeight: 500, color: "var(--text-01)", marginBottom: "8px" }}>RGF Anexo 02</p>
                <p style={S.body}>Dívida Consolidada — Relatório de Gestão Fiscal. Base: 3º quadrimestre (encerramento do exercício).</p>
              </div>
              <div>
                <p style={{ fontSize: "14px", fontWeight: 500, color: "var(--text-01)", marginBottom: "8px" }}>Referência legal</p>
                <p style={S.body}>
                  Lei Complementar 101/2000 (LRF) · Resolução do Senado Federal 40/2001 (limite de dívida) ·
                  Nota STN sobre limite de alerta (90% do limite máximo).
                </p>
              </div>
            </div>
          </div>
