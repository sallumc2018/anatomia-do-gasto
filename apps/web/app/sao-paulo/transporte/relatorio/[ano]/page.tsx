...
export async function generateMetadata({ params }: { params: Promise<{ ano: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Despesas em transporte — ${ano} | São Paulo`,
    description: `Execução orçamentária em transporte de São Paulo em ${ano}: dotação atualizada, empenhada, liquidada e paga. Fonte: SICONFI/Tesouro Nacional — RREO Anexo 02 e DCA Anexo I-E.`,
    alternates: { canonical: `https://www.anatomiadogasto.ong.br/sao-paulo/transporte/relatorio/${ano}` },
  }
}
...
export default async function TransporteRelatorioPage({ params }: PageProps) {
  ...
  const orcamento = loadTransporteOrcamento(year, MUNICIPIO)
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <JsonLd data={{
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Transporte", item: "https://www.anatomiadogasto.ong.br/sao-paulo/transporte" },
      ...
      <JsonLd data={{
        "@context": "https://schema.org",
        "@type": "Dataset",
        name: `Execução orçamentária em transporte — São Paulo ${year}`,
        description: `Execução orçamentária em transporte de São Paulo em ${year}: dotação atualizada, empenhada, liquidada e paga. Fonte: SICONFI/Tesouro Nacional — RREO Anexo 02 e DCA Anexo I-E.`,
        url: `https://www.anatomiadogasto.ong.br/sao-paulo/transporte/relatorio/${year}`,
      ...
      <main id="conteudo" className="flex-1">
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)" }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
            <p style={S.label} className="mb-3">Função 26 — Transporte · São Paulo/SP</p>
            ...
            <p className="mt-4" style={{ fontSize: "13px", color: "var(--text-04)", maxWidth: "640px" }}>
              <strong style={{ color: "var(--text-03)" }}>Limitação desta fonte:</strong> São Paulo declara
              toda a função 26 sem quebra por subfunção nestes dados. Os valores abaixo incluem transporte
              coletivo urbano (ônibus, corredores, subsídio ao sistema), obras viárias e demais subfunções
              de mobilidade sem discriminação.
            </p>
            <div className="flex items-center gap-3 mt-5">
              <span style={{ fontSize: "11px", color: "var(--text-03)", fontFamily: "var(--font-ibm-plex-mono)" }}>Baixar dados brutos:</span>
              <a
                href={`/api/dados/sao_paulo/transporte/saida/rreo_transporte_sao_paulo_${year}.csv`}
                download
              ...
              {dca && (
                <a
                  href={`/api/dados/sao_paulo/transporte/saida/dca_transporte_sao_paulo_${year}.csv`}
                  download
                  style={{ fontSize: "12px", fontFamily: "var(--font-ibm-plex-mono)", color: "var(--blue-40)", textDecoration: "none", border: "1px solid var(--border-01)", padding: "4px 10px", whiteSpace: "nowrap" }}
                >
                  ↓ DCA
              ...
              <>
                ...
                {year === 2026 && (
                  <p className="mb-6" style={{ fontSize: "13px", color: "var(--text-04)", maxWidth: "640px" }}>
                    <strong style={{ color: "var(--text-03)" }}>Exercício em curso:</strong> o RREO {year}
                    reflete a execução parcial publicada até o 2º bimestre, não o fechamento do ano. Os
                    valores tendem a crescer nos próximos bimestres.
                  </p>
                )}
          ...
          <div className="mx-auto px-6 py-12" style={S.container}>
            ...
            {dca ? (
              <>
                ...
                {orcamento && Math.round(dca.empenhado * 100) === Math.round(orcamento.empenhado * 100) && (
                  <p className="mb-6" style={{ fontSize: "13px", color: "var(--text-04)", maxWidth: "640px" }}>
                    <strong style={{ color: "var(--text-03)" }}>Consistência verificada:</strong> o Empenhado
                    do DCA bate com o Empenhado (exceto intra) do RREO Anexo 02 acima — nenhuma
                    inconsistência entre as duas fontes para {year}.
                  </p>
                )}
              ...
              <p style={S.body}>
                Dado DCA não disponível para {year}
                {year === 2026 && " — o DCA Anexo I-E é um demonstrativo de encerramento anual, publicado apenas após o fim do exercício"}.
              </p>
            )}
          </div>
        </section>
        ...
        <section style={{ backgroundColor: "var(--bg-base)" }}>
          <div className="mx-auto px-6 py-10" style={S.container}>
            ...
            <ul style={{ ...S.body, listStyle: "disc", paddingLeft: "24px", display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <li>Operação (frota, linhas, passageiros) — requer dados abertos de SPTrans/SP Mobilidade</li>
              <li>Contratos do serviço — PNCP e portal de transparência municipal, requer curadoria adicional</li>
              <li>Serviços metropolitanos/intermunicipais (Metrô, CPTM, EMTU) — orçamento estadual, fora do escopo municipal</li>
            </ul>
          </div>
        </section>
        ...
        <section style={{ backgroundColor: "var(--bg-elevated)", borderTop: "1px solid var(--border-01)" }}>
          <div className="mx-auto px-6 py-6 flex justify-between items-center" style={S.container}>
            {hasPrev ? (
              <Link href={`/sao-paulo/transporte/relatorio/${prevYear}`}
                    style={{ color: "var(--blue-60)", fontSize: "14px" }}>
                ← {prevYear}
              </Link>
            ) : <span />}
            <Link href="/sao-paulo/transporte/comparativo"
                  style={{ color: "var(--text-03)", fontSize: "13px" }}>
              Ver comparativo histórico
            </Link>
            {hasNext ? (
              <Link href={`/sao-paulo/transporte/relatorio/${nextYear}`}
                    style={{ color: "var(--blue-60)", fontSize: "14px" }}>
                {nextYear} →
              </Link>
            ) : <span />}
