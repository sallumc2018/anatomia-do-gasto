...
const SUBFUNCAO_TIPS: Record<string, string> = {
  "06.122 - Administração Geral":
    "Salários, administração interna e custos operacionais de base da Guarda Civil Metropolitana.",
  "06.181 - Policiamento":
  ...
  "06.999 - Demais Subfunções Segurança Pública":
    "Rubrica agregadora usada pelo DCA em 2015–2016 para fechar o total da função 06 sem detalhamento adicional.",
}
...
export async function generateMetadata({ params }: { params: Promise<{ ano: string }> }): Promise<Metadata> {
  ...
  return {
    title: `Despesas em segurança pública — ${ano} | São Paulo`,
    description: `Execução orçamentária em segurança pública de São Paulo em ${ano}: dotação atualizada, empenhada, liquidada e paga. Fonte: SICONFI/Tesouro Nacional — DCA e RREO.`,
    alternates: { canonical: `https://www.anatomiadogasto.ong.br/sao-paulo/seguranca/relatorio/${ano}` },
  }
}

export default async function RelatorioSegurancaSaoPauloPage({ params }: PageProps) {
  const { ano } = await params
  ...
  const somaDetalhada = detail.reduce((acc, r) => acc + r.liquidada, 0)
  const naoDetalhado = total ? Math.max(0, total.liquidada - somaDetalhada) : 0
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <JsonLd data={{
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Segurança Pública", item: "https://www.anatomiadogasto.ong.br/sao-paulo/seguranca" },
          { "@type": "ListItem", position: 3, name: "Série histórica", item: "https://www.anatomiadogasto.ong.br/sao-paulo/seguranca/comparativo" },
      ...
      <JsonLd data={{
        "@context": "https://schema.org",
        "@type": "Dataset",
        name: `Execução orçamentária em segurança pública — São Paulo ${year}`,
        description: `Execução orçamentária em segurança pública de São Paulo em ${year}: dotação atualizada, empenhada, liquidada e paga por subfunção. Fonte: SICONFI/Tesouro Nacional.`,
        url: `https://www.anatomiadogasto.ong.br/sao-paulo/seguranca/relatorio/${year}`,
        ...
        spatialCoverage: { "@type": "Place", name: "São Paulo, São Paulo, Brasil" },
      }} />
      <main id="conteudo" className="flex-1">
        ...
        <div style={{ backgroundColor: "var(--bg-base)", ...S.borderBottom }}>
          <div className="mx-auto px-6 py-3 flex items-center gap-2" style={{ ...S.container, flexWrap: "wrap" }}>
            ...
            <Link href="/sao-paulo/seguranca" className="nav-link" style={{ fontSize: "12px" }}>Segurança</Link>
            <span style={{ fontSize: "12px", color: "var(--text-04)" }}>/</span>
            <Link href="/sao-paulo/seguranca/comparativo" className="nav-link" style={{ fontSize: "12px" }}>Série histórica</Link>
            <span style={{ fontSize: "12px", color: "var(--text-04)" }}>/</span>
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                <p className="uppercase font-semibold mb-3" style={S.label}>
                  São Paulo / SP · Segurança Pública · Exercício completo
                </p>
              ...
              <div className="mobile-scroll flex items-center gap-3">
                {availableYears.map((y) => (
                  <Link key={y} href={`/sao-paulo/seguranca/relatorio/${y}`} style={{
                    fontSize: "13px",
                ...
                <a
                  href={`/api/dados/sao_paulo/seguranca/saida/despesas_seguranca_sao_paulo_${year}.csv`}
                  download
          ...
          <section style={{ backgroundColor: "var(--bg-elevated)", ...S.borderBottom }}>
            <div className="mx-auto px-6 py-8" style={S.container}>
              <p className="uppercase font-semibold mb-6" style={S.label}>
                DCA Anexo I-E · Exercício {year} · São Paulo/SP
              </p>
        ...
        {/* ── Nota: resíduo não detalhado ──────────────────────────────────── */}
        {naoDetalhado > 0 && total && (
            ...
            <div className="mx-auto px-6 py-6" style={S.container}>
              <div className="p-5" style={{ backgroundColor: "var(--bg-elevated)", border: "1px solid var(--border-01)", borderLeft: "4px solid #f1c21b" }}>
                <p className="font-semibold mb-2" style={{ color: "var(--text-01)", fontSize: "13px" }}>
                  Parte do total não está detalhada por subfunção em {year}
                </p>
                <p style={{ ...S.small, color: "var(--text-03)" }}>
                  A soma das subfunções declaradas abaixo é {formatPrecise(somaDetalhada)}, mas o total
                  liquidado da função 06 informado no DCA é {formatPrecise(total.liquidada)}.
                  A diferença de {formatPrecise(naoDetalhado)} ({pct(naoDetalhado, total.liquidada)} do total)
                  não está atribuída a nenhuma subfunção nos dados publicados pelo SICONFI para este exercício.
                  Isso está presente no dado bruto extraído da API — não é um erro de processamento deste site.
                </p>
              </div>
            </div>
          </section>
        ...
        <section style={{ backgroundColor: "var(--bg-base)" }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
              ...
              <div className="table-scroll">
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    ...
                    {naoDetalhado > 0 && total && (
                      <tr style={S.borderBottom}>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", color: "var(--text-03)", fontStyle: "italic" }}>
                          Não detalhado por subfunção
                          <p style={{ ...S.caption, marginTop: "2px" }}>diferença entre o total da função 06 e a soma das subfunções acima</p>
                        </td>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", textAlign: "right", color: "var(--text-04)" }}>—</td>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", textAlign: "right", color: "#f1c21b", fontFamily: "var(--font-ibm-plex-mono)" }}>
                          {formatPrecise(naoDetalhado)}
                        </td>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", textAlign: "right", color: "var(--text-04)" }}>—</td>
                        <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", textAlign: "right", color: "var(--text-03)", fontFamily: "var(--font-ibm-plex-mono)" }}>
                          {pct(naoDetalhado, total.liquidada)}
                        </td>
                        {prevYear && <td style={{ padding: "14px 0 14px 0" }} />}
                      </tr>
                    )}
                  </tbody>
                  {total && (
            ...
            <p className="mt-8" style={S.caption}>
              Fonte: DCA Anexo I-E — SICONFI / Tesouro Nacional. São Paulo/SP · Exercício {year}.
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div>
                ...
                <ul className="flex flex-col gap-3">
                  {[
                    "Quanto foi empenhado, liquidado e pago em cada subfunção da segurança pública declarada.",
              ...
              <div>
                ...
                <ul className="flex flex-col gap-3">
                  {[
                    ...
                    "A que subfunção pertence o valor \"não detalhado\" (ver nota acima, quando aplicável a este exercício).",
