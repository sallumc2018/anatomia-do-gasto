...
import {
  ...
  FUNCAO_LABELS,
  type HealthRow,
  type RREODespesasRow,
  type RREOReceitasRow,
} from "@/lib/format"

// ── Style constants (mesmas do page.tsx) ─────────────────────────────────────
...
export interface QuadriSelectorProps {
  year:          number
  quads:         number[]
  defaultQuad:   number
  allData:       HealthRow[]
  prevYear:      number | null
  prevAllData:   HealthRow[]
  rreoAll:       RREODespesasRow[]
  rreoRecAll:    RREOReceitasRow[]
  totalRow:      string   // TOTAL_ROW["saude"]
}

// ── Component ─────────────────────────────────────────────────────────────────

export function QuadriSelector({
  year,
  quads,
  defaultQuad,
  allData,
  ...
  const quad   = quads.includes(Number(qParam)) ? Number(qParam) : defaultQuad

  // Derivar dados do quadrimestre ativo
  const rows   = allData.filter((r) => r.quadrimestre === quad)
  const total  = rows.find((r) => r.funcao === totalRow)
  const detail = rows.filter((r) => r.funcao !== totalRow)

  const prevRows = prevAllData.filter((r) => r.quadrimestre === quad)
  ...
  function handleTabClick(q_: number, e: React.MouseEvent<HTMLAnchorElement>) {
    e.preventDefault()
    const url = q_ === defaultQuad ? pathname : `${pathname}?q=${q_}`
      ...
      {total && (
        <RastroDinheiro
          ...
          destinations={detail.map((row) => ({
            label: FUNCAO_LABELS["saude"][row.funcao] ?? row.funcao,
            value: row.liquidada,
          }))}
        />
      )}
                  ...
                  {detail.map((row) => (
                    <tr key={row.funcao} style={{ ...S.borderBottom }}>
                      <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", color: "var(--text-01)" }}>
                        {FUNCAO_LABELS["saude"][row.funcao] ?? row.funcao}
                ...
                {total && (
                  <tfoot>
                    <tr style={{ borderTop: "2px solid var(--border-02)" }}>
                      ...
                      {prevYear && (
                        <Delta current={total.liquidada} prev={prevByFuncao[totalRow]} bold />
                      )}
                    </tr>
                  </tfoot>
                )}
