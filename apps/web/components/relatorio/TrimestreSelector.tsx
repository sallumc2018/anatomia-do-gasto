...
export interface TrimestreSelectorProps {
  year:          number
  trims:         number[]
  defaultTrim:   number
  allData:       HealthRow[]
...
export function TrimestreSelector({
  ...
  const trim   = trims.includes(Number(tParam)) ? Number(tParam) : defaultTrim

  // Derivar dados do trimestre ativo
  const rows   = allData.filter((r) => r.quadrimestre === trim)
  const total  = rows.find((r) => r.funcao === TOTAL_ROW["educacao"])
  const detail = rows.filter((r) => r.funcao !== TOTAL_ROW["educacao"])

  const prevRows = prevAllData.filter((r) => r.quadrimestre === trim)
  ...
  function handleTabClick(t_: number, e: React.MouseEvent<HTMLAnchorElement>) {
    e.preventDefault()
    const url = t_ === defaultTrim ? pathname : `${pathname}?t=${t_}`
      ...
      {total && (
        <RastroDinheiro
          ...
          destinations={detail.map((row) => ({
            label: FUNCAO_LABELS["educacao"][row.funcao] ?? row.funcao,
            value: row.liquidada,
          }))}
        />
      )}
                  ...
                  {detail.map((row) => (
                    <tr key={row.funcao} style={S.borderBottom}>
                      <td style={{ padding: "14px 16px 14px 0", fontSize: "13px", color: "var(--text-01)" }}>
                        {FUNCAO_LABELS["educacao"][row.funcao] ?? row.funcao}
                ...
                {total && (
                  <tfoot>
                    <tr style={{ borderTop: "2px solid var(--border-02)" }}>
                      ...
                      {prevYear && <Delta current={total.liquidada} prev={prevByFuncao[TOTAL_ROW["educacao"]]} bold />}
                    </tr>
                  </tfoot>
                )}
              </table>
