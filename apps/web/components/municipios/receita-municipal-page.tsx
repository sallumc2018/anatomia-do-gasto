...
export interface ReceitaMunicipalPageConfig {
  ...
  compositionParagraphs?: readonly string[]
  transferInsightSuffix?: string
  rppsLabel: string
  showIntraWhenZero?: boolean
...
function buildSerie(anos: number[], municipio: string): SerieAnual[] {
  return anos
    .map((ano) => {
...
export async function ReceitaMunicipalPage({
  searchParams,
  config,
}: {
  searchParams: Promise<{ ano?: string }>
  config: ReceitaMunicipalPageConfig
}) {
  const params = await searchParams
  const anos = getAvailableYearsReceita(config.dataMunicipio)
  ...
  const dados = loadReceitaMunicipal(anoFeatured, config.dataMunicipio)
  const serie = buildSerie(anos, config.dataMunicipio)
  ...
  const insights: string[] = [
    ...
    ...(growthSerie !== null
      ...
      : []),
    `As transferências intergovernamentais representam ${depTransf.toFixed(1)}% das receitas exceto intra-orçamentárias${config.transferInsightSuffix ?? "."}`,
          ...
          <div className="mx-auto px-6 py-16 md:py-24" style={S.container}>
            <div className="mobile-hero-inset" style={{ borderLeft: "4px solid var(--blue-60)", paddingLeft: "24px" }}>
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <p className="uppercase font-semibold" style={S.label}>Receita Municipal · {config.nome}/SP</p>
              ...
              <p style={{ ...S.body, maxWidth: "640px", marginBottom: "12px" }}>
                Em {anoFeatured}, {config.nome} arrecadou{" "}
              ...
              {config.contextParagraphs?.map((paragraph) => (
                <p
                  key={paragraph}
                  style={{ ...S.body, maxWidth: "640px", color: "var(--text-03)", marginBottom: "12px" }}
                >
                  {paragraph}
                </p>
              ))}
              ...
              <p style={S.caption}>Fonte: SICONFI/Tesouro Nacional — RREO Anexo 01 · 6º bimestre · IBGE {config.ibge}</p>
            </div>
          </div>
        </section>
        ...
        <section style={{ backgroundColor: "var(--bg-base)", ...S.borderBottom }}>
          <div className="mx-auto px-6 py-4" style={S.container}>
            <div className="flex flex-wrap items-center gap-4">
              ...
              <AnoSelector anos={anos} selectedAno={anoFeatured} basePath={`/${config.slug}/receita`} />
            </div>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-12">
              <div>
                ...
                <p style={{ ...S.body, marginBottom: "16px" }}>
                  O gráfico mostra a composição da receita corrente de {config.nome} em {anoFeatured}.
                </p>
                {config.compositionParagraphs?.map((paragraph) => (
                  <p key={paragraph} style={{ ...S.body, color: "var(--text-03)", marginBottom: "16px" }}>
                    {paragraph}
                  </p>
                ))}
                {(config.showIntraWhenZero || intra > 0) && (
                  <div className="p-4" style={{ backgroundColor: "var(--bg-elevated)", border: "1px solid var(--border-01)" }}>
                    <p style={{ ...S.caption, lineHeight: "18px" }}>
                      ...
                      {fmt(intra)} são receitas intra-orçamentárias — contribuições previdenciárias dos servidores ao {config.rppsLabel}.
                      São computadas à parte por representar circulação interna.
                    </p>
                  </div>
                )}
              </div>
        ...
        <section id="metodologia" style={{ backgroundColor: "var(--bg-elevated)", ...S.borderBottom }}>
          <div className="mx-auto px-6 py-12" style={S.container}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div>
                <p className="uppercase font-semibold mb-3" style={S.label}>{config.methodologyTitle}</p>
                <p style={S.body}>
