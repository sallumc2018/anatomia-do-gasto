...
ROUTES = [
    (
        re.compile(r"(^/goal\b|\bgoal\b|objetivo amplo|criterio de sucesso|critério de sucesso|plano verificavel|plano verificável)", re.I),
        ["goal", "maestro"],
        "Define success criteria and package before Maestro routes the task.",
    ),
    (
        re.compile(r"(auditoria|auditar|cobertura|reconcili).*(sorocaba|public|publica|publicacao|manifest)|auditoria_cobertura_sorocaba", re.I),
        ["pipeline", "qa"],
        "Coverage/publication audits are generated by pipeline and checked read-only by QA.",
    ),
    (
        re.compile(r"(validar|qa|verificar_publicacao|integridade).*(public|publica|publicacao|manifest)", re.I),
        ["qa"],
        "Publication QA is read-only and can inspect data/public plus manifests.",
    ),
    (
        re.compile(r"(completar|faltant|lacuna|ausente).*(dado|dados)|dados.*(faltant|ausente|lacuna)", re.I),
        ["frontino", "dados", "pipeline", "qa", "frontend?", "deploy?"],
        "Complete missing published coverage without publication until QA and explicit approval.",
    ),
    (
        re.compile(r"(schema|dba|modelo de dados|consistencia|consistÃªncia|normaliza|migracao de dados|migraÃ§Ã£o de dados)", re.I),
        ["dba"],
        "Schema and data-model consistency start with DBA in read-only mode.",
    ),
    (
        re.compile(r"(analisar|analise|anÃ¡lise|percentual|execucao|execuÃ§Ã£o|comparar|relatorio|relatÃ³rio|cifra|insight|linguagem cidad)", re.I),
        ["analista"],
        "Published-data interpretation starts with the analyst.",
    ),
    (
    ...
    (
        re.compile(r"(instalar|dependencia|dependÃªncia|npm " + "install|npm " + "update|npm audit|npx)", re.I),
        ["seguranca", "frontend?"],
        "Dependency changes start with security triage and require human approval.",
    ),
    (
