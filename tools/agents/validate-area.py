...
def get_commands(area: str) -> list[dict]:
    ...
    if area == "all":
        commands: list[dict] = []
        for item in ("memory", "agents", "scope", "secrets", "review", "pipeline", "frontend"):
...
def main() -> int:
    ...
    payload = {
        ...
        "status": status,
        "results": results,
        "forbidden_actions": ["push", "deploy", "npm install", "data/public publication without explicit approval"],
    }
