"""
QA estrutural dos CSVs publicados em data/public/paulinia/.

Verifica cada CSV: parseável, tem header, tem dados, sem colunas 100% vazias,
campos monetários/anuais com fill rate razoável.

Uso:
    .venv/bin/python3 tools/data/qa_public_paulinia.py
    .venv/bin/python3 tools/data/qa_public_paulinia.py --strict   # exit 1 se houver fail
    .venv/bin/python3 tools/data/qa_public_paulinia.py --area camara
    .venv/bin/python3 tools/data/qa_public_paulinia.py --json
"""
...
PUBLIC_ROOT = ROOT / "data" / "public" / "paulinia"
...
MIN_ROWS = 1
SMALL_FILE_OK: set[str] = set()

# Padrões onde fill rate baixo é esperado → rebaixar fail → warn
SPARSE_OK_PATTERNS = (
    "catalogo_",               # índice de PDFs: campos como download_url podem estar vazios
    "inventario_",             # TCE inventário: ano_referencia_publicacao esparso
    "links_relevantes_",       # links TCE: sem valor/cnpj
    "alertas_tce_",            # alertas TCE: sem valor monetário
    "inventario_fontes_",      # fontes TCE: metadados
    "pncp_paulinia_compras",   # PNCP compras diretas: valor_global null por design da API
    "pncp_paulinia_atas",      # PNCP atas de registro: valor_global null por design da API
    "pncp_paulinia_2022_2026",  # PNCP consolidado mistura compras/atas + contratos
    "fnde_repasses_",          # repasses do FNDE podem conter placeholders ou dados esparsos
    "empenhos_fornecedores_paulinia_2026",  # ano corrente: empenhos sem pagamento ainda
)
...
def main() -> int:
    ...
    total = summary["total"]
    print(f"qa_public_paulinia: {len(results)} arquivos — "
