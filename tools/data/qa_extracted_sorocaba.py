...
def write_markdown(results: list[FileQA], output: Path, report_date: str) -> None:
    summary = summarize(results)
    lines = [
        f"# QA extracted Sorocaba - {report_date}",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--date", default=date.today().isoformat(), help="Data ISO usada no relatorio markdown.")
    args = parser.parse_args()

    results = scan()
    if args.markdown:
        write_markdown(results, args.markdown, args.date)
