...
def generate_coverage_section(existing_date: str = "") -> str:
    ...
    date_str = existing_date if existing_date else get_last_commit_date()
    ...
    lines = [
        ...
        f"- **Datasets em validacao:** {validated}",
        f"- **Atualizado em:** {date_str}",
    ]
...
def update_readme(filepath: Path, include_activity: bool = True, write: bool = True) -> bool:
    """
    ...
    # Tenta extrair a data existente do README para evitar falhas no CI decorrentes apenas da data de commit
    existing_date = ""
    date_match = re.search(r"-\s*\*\*Atualizado em:\*\*\s*(\d{4}-\d{2}-\d{2})", original)
    if date_match:
    ...
    for match in auto_pattern.finditer(original):
        ...
        if tag_name == "coverage":
            new_body = generate_coverage_section(existing_date if not write else "")
...
def process_readmes(check_mode: bool = False) -> bool:
    ...
    for readme in readme_files:
        if readme.exists():
            modified = update_readme(readme, include_activity=include_activity, write=not check_mode)
            if modified:
