"""
Saneador LGPD — mascara CPF de pessoas físicas em data/public (CSV e JSON).
...
recurso público é divulgável, CPF deve ser protegido). Este saneador:

  1. Preserva o arquivo ORIGINAL (com CPF completo) em data/private/lgpd_reservado/
...
arquivo já mascarado não faz nada (não há CPF completo a mascarar).
...
def main() -> int:
    ...
    ap.add_argument("--gate", action="store_true",
                    help="modo gate: não altera; sai com código 1 se houver QUALQUER CPF "
    ...
    for arq_path in _coletar_arquivos():
        try:
            ...
            continue
        if not _RE_CPF.search(texto) and not _RE_CPF_PURO.search(texto):
            continue
    ...
    print(f"\n{modo}{total_cpf} CPF(s) em {total_arquivos} arquivo(s).")
    ...
    if args.gate and total_cpf > 0:
        print("\n❌ GATE: CPF detectado em data/public. Rode:")
