...
from paths import MUNICIPIO, CFG, DATA_DIR

EXTRACTED = DATA_DIR / "extracted" / MUNICIPIO / "pncp" / "saida"
PUBLIC = DATA_DIR / "public" / MUNICIPIO / "contratos" / "saida"
EXPECTED_ORGAO_CNPJ = CFG["cnpj_prefeitura"]
...
def _is_target_orgao(row: dict[str, str]) -> bool:
    return (row.get("orgao_cnpj") or "").strip() == EXPECTED_ORGAO_CNPJ
...
def ler_fonte(fonte: Path, vistos: set, registros: list) -> int:
    ...
    with fonte.open(encoding=_enc(fonte), newline="") as f:
        for row in csv.DictReader(f):
            if not _is_target_orgao(row):
                _skip("orgao_cnpj_fora_de_municipio")
                continue
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Consolida PNCP Sorocaba sem publicar por padrao.")
    parser.add_argument(
        "--write-public",
        action="store_true",
        help="Escreve em data/public. Exige autorizacao explicita do operador.",
    )
...
def main() -> None:
    ...
    legado = [
        EXTRACTED / f"pncp_{MUNICIPIO}_atas-compras-contratos_20220101_20261231.csv",
        EXTRACTED / f"pncp_{MUNICIPIO}_atas_20200101_20260521.csv",
        EXTRACTED / f"pncp_{MUNICIPIO}_20200101_20260520.csv",
    ]
    for fonte in legado:
        if not fonte.exists():
    ...
    for tipo in tipos:
        for ano in anos:
            fonte = EXTRACTED / f"pncp_{MUNICIPIO}_{tipo}_{ano}.csv"
    ...
    out = PUBLIC / f"pncp_{MUNICIPIO}_2022_2026.csv"
    ...
    action = "Publicado" if args.write_public else "Dry-run"
    print(f"\n{action}: {len(registros)} registros unicos -> {out}")
    print(f"Por dataset: {dict(sorted(por_dataset.items()))}")
    print(f"Por ano: {dict(sorted(por_ano.items()))}")
    print(f"Ignorados: {dict(sorted(SKIPPED.items()))}")
