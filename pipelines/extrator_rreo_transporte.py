"""
Extrai orçamento de Transporte de Sorocaba do SICONFI RREO-Anexo 02.
...
Função orçamentária 26 — Transporte.

Limitação crítica — subfunção única:
    O RREO 2024 apresenta Transporte em uma única subfunção
    "FU26 - Demais Subfunções". Sorocaba não discrimina, dentro da função 26,
    o serviço de transporte público urbano de obras de infraestrutura viária.
    O valor total é auditável; a decomposição interna não existe nos dados federais.

Metodologia EXCETO INTRA vs INTRA:
    Mesma abordagem que extrator_rreo_seguranca.py — marcador de seção.
    Empenhado = valor EXCETO INTRA (componente principal).
    Intra_Empenhado = componente INTRA (dado de auditoria).

Relação DCA × RREO:
    2024: DCA Empenhado R$575.57M = RREO EXCETO R$575.57M (padrão esperado).
    Padrão verificado apenas para 2024 — anos anteriores requerem validação
    individual pelo extrator_dca_transporte.py.

Taxa de execução 2024 (~99,7%):
    Empenhado (R$575M) / Dotação Atualizada (R$577M) muito próximo de 100%.
    Indica possível replanejamento orçamentário ao longo do ano (créditos
    adicionais acompanhando a execução real). Não indica necessariamente
    eficiência — requer análise de série histórica para contextualizar.

Pipeline:
    data/raw/sorocaba/transporte/entrada/{ano}_rreo_anexo02_b6_siconfi.json
    data/extracted/sorocaba/transporte/saida/rreo_transporte_sorocaba_{ano}.csv
...
Uso:
    python extrator_rreo_transporte.py           # 2020-2025
    python extrator_rreo_transporte.py --ano 2024
    python extrator_rreo_transporte.py --use-raw
"""
import argparse
import csv
import json
...
def processar_ano(ano: int, use_raw: bool = False) -> bool:
    ...
    dot_atu = resultado["dotacao_atualizada"]
    taxa = (emp_exc / dot_atu * 100) if dot_atu > 0 else 0.0

    print(f"  Dotacao Inicial:     R$ {br_format(resultado['dotacao_inicial'])}")
    print(f"  Dotacao Atualizada:  R$ {br_format(dot_atu)}")
    print(f"  Empenhado EXCETO:    R$ {br_format(emp_exc)}")
    print(f"  Empenhado INTRA:     R$ {br_format(emp_int)}")
    print(f"  Taxa de execucao:    {taxa:.1f}%"
          + ("  [ATIPICO: >95%]" if taxa > 95 else ""))
...
def main():
    parser = argparse.ArgumentParser(
        description="Extrai orcamento de Transporte do SICONFI RREO Anexo 02"
    )
