...
from paths import as_str, SAUDE_EXTRACTED_DIR, SAUDE_PUBLIC_DIR, SAUDE_VALIDATED_DIR
...
def escolher_csv_saude(nome):
    candidatos = [
        SAUDE_PUBLIC_DIR / "saida" / nome,
        SAUDE_VALIDATED_DIR / "saida" / nome,
        SAUDE_EXTRACTED_DIR / "saida" / nome,
    ]
    for path in candidatos:
        if path.exists():
            return as_str(path)
    return as_str(candidatos[0])
...
def verificar_receitas(ano):
    ...
    csv_path = escolher_csv_saude(f'receitas_base_saude_sorocaba_{ano}.csv')
...
def verificar(ano):
    ...
    intermediario_dir = as_str(SAUDE_EXTRACTED_DIR / "intermediario")
    csv_path          = escolher_csv_saude(f'despesas_saude_sorocaba_{ano}.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ano', type=int, action='append', default=None, help='Ano a verificar (pode repetir)')
    args = parser.parse_args()
    anos = args.ano or [2025]
    ok = all(verificar(ano) and verificar_receitas(ano) for ano in anos)
