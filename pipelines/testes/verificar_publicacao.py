...
def public_csv_path(municipio: str, area: str, filename: str) -> Path:
    return PUBLIC_ROOT / municipio / area_path(area) / "saida" / filename
...
def verificar() -> list[str]:
    erros: list[str] = []
    publicaveis: dict[tuple[str, str], list[re.Pattern[str]]] = {}
    nao_publicaveis: dict[tuple[str, str], list[tuple[re.Pattern[str], str]]] = {}
    ...
    with open(MANIFEST, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if origem == "public_aux":
                ...
                continue
            if origem != "public":
                nao_publicaveis.setdefault((municipio, area), []).append((padrao_para_regex(padrao), origem))
                anos = anos_do_manifesto(area, padrao, anos_str, erros)
                for ano in anos:
                    nome = padrao.replace("{ano}", str(ano))
                    caminho = public_csv_path(municipio, area, nome)
                    if caminho.exists():
                        erros.append(
            ...
            publicaveis.setdefault((municipio, area), []).append(padrao_para_regex(padrao))
            anos = anos_do_manifesto(area, padrao, anos_str, erros)
    ...
    municipios = sorted({municipio for municipio, _area in publicaveis} | {municipio for municipio, _area in nao_publicaveis})
    for municipio in municipios:
        municipio_root = PUBLIC_ROOT / municipio
        if not municipio_root.exists():
            continue
        for caminho in sorted(municipio_root.rglob("*.csv")):
            partes = caminho.relative_to(municipio_root).parts
            ...
            nome = caminho.name
            if any(regex.match(nome) for regex in publicaveis.get((municipio, area), [])):
                continue

            for regex, origem in nao_publicaveis.get((municipio, area), []):
                if regex.match(nome):
                    erros.append(
                        f"NAO PUBLICAVEL EM data/public: {caminho.relative_to(ROOT)} "
                        f"(Origem_Dir={origem})"
