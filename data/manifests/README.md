...
Em expansao municipal, cada linha de `datasets.csv` deve ter classificacao correspondente pelo par `municipio` + `area` + `tipo` + `arquivo_padrao`. Linhas com `Origem_Dir=gap`, `validated` ou `extracted` podem registrar LAI, protocolo ou lacuna, mas continuam fora da UI publica ate promocao explicita para `data/public`.
