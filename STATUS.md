...
- `emendas_federais` extrator 🔴 BUG DE ARQUITETURA CONFIRMADO 2026-07-09 (via swagger oficial `api.portaldatransparencia.gov.br/v3/api-docs`): endpoint `/api-de-dados/emendas` **não tem filtro por município** (só `codigoEmenda`, `numeroEmenda`, `nomeAutor`, `tipoEmenda`, `ano`, `codigoFuncao`, `codigoSubfuncao`, `pagina`). Script antigo usava `localidadeGasto`/`anoExercicio` — parâmetros inválidos, ignorados silenciosamente pela API; confirmado que os **77 municípios publicados têm as mesmas 52 linhas (emenda,ano) idênticas**, só relabeladas com IBGE errado. `pipelines/baixar_emendas_federais.py` reescrito: usa só `ano`+`pagina` reais, cache nacional compartilhado em `data/raw/_nacional/emendas_federais/`, filtra por `localidadeDoGasto` no cliente antes de gravar (9 testes novos/atualizados, todos verdes). **NÃO VERIFICADO com chave real** (PORTAL_TRANSPARENCIA_KEY inválida/bloqueada — mesmo blocker de rate limit já registrado). Também não é possível confirmar se valores deixam de vir zerados numa consulta corretamente filtrada (pode ser execução fundo-a-fundo tracked fora do registro de emenda). **Dados publicados em `data/public/*/emendas_federais/` continuam errados e não devem ser usados** — aguardando decisão do usuário sobre quarentena/remoção (não removidos unilateralmente).
