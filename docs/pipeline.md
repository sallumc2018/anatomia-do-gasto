...
```powershell
$env:ANATOMIA_RAW_ROOT = "G:\Meu Drive\02-Profissional\03-Big-Data-Fiscal-Data\raw"
```

Com essa variavel, os scripts que usam `pipelines/paths.py` leem e gravam brutos em `$env:ANATOMIA_RAW_ROOT\sorocaba\...`, enquanto `data/extracted`, `data/validated` e `data/public` continuam no repo. No Windows operacional atual, o destino canonico do acervo bruto e `G:\Meu Drive\02-Profissional\03-Big-Data-Fiscal-Data\raw`.
...
`verificar_publicacao.py` usa o campo `municipio` de `data/manifests/datasets.csv` para validar `data/public/<municipio>/<area>/saida`. Nao assuma Sorocaba como raiz unica quando adicionar ou revisar municipios.
...
Para expansao segura da ONG, siga tambem `docs/protocolo-dados-reais-expansao.md`.
Antes de publicar ou remover dados publicos, rode:
...
Delecoes em `data/public` sao bloqueadas por padrao pelo gate de commit. Uma remocao
deliberada exige decisao documentada e `ANATOMIA_ALLOW_PUBLIC_DELETE=1`.

`pipelines\gerar_pncp_publicacao.py` consolida candidatos PNCP e roda em dry-run por padrao. A escrita em `data/public/sorocaba/contratos/saida/pncp_sorocaba_2022_2026.csv` exige `--write-public` e autorizacao explicita; o script filtra `orgao_cnpj=46634044000174` para evitar registros de orgaos fora de Sorocaba.
